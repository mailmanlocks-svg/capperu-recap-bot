...
def build_recap(target_date, all_picks):
    ...
    display_date = date.fromisoformat(target_date).strftime("%-m/%-d")
    ...
    lines.append("<@&1425232193361547284>")
...
def build_upsell(tier_num, sorted_partners, total_u, total_w, total_l, sweepers):
    ...
    lines += ["", giveback, "We're dropping a limited deal to celebrate 👇", "", f"💎 **{tier['discount']} — {tier['label']}** 💎", "", scarcity, f"🎟️ Code: `{tier['code']}`", f"👉 {tier['link']}", "", closer]
    return "\n".join(lines)
